...
@Injectable()
  ...
  @Component({
    selector: 'app-bterpendingto-submit-application',
    templateUrl: './bterpendingto-submit-application.component.html',
    styleUrls: ['./bterpendingto-submit-application.component.css']
  })
export class BTERPendingtoSubmitApplicationComponent implements OnInit {
  ...
  public TotalBTERPendingtoSubmittedApplicationList: any = [];
  ...
  async ngOnInit() {
    ...
    await this.GetTotalBterPendingtoSubmittedApplicationList();
  }
  ...
  async GetTotalBterPendingtoSubmittedApplicationList() {
    debugger;
    try {
      this.loaderService.requestStarted();
      this.request.DepartmentID = this.sSOLoginDataModel.DepartmentID;
      await this.collegeservice.TotalBTERApplicationDetailsByDepartment(this.request, this.sSOLoginDataModel.SessionID, 'PendingtoSubmit')
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          this.TotalBTERPendingtoSubmittedApplicationList = data['Data'][0]['data'];
  ...
  btnExportTable_Click(): void {
    this.loaderService.requestStarted();
    if (this.TotalBTERPendingtoSubmittedApplicationList.length > 0) {
      try {
