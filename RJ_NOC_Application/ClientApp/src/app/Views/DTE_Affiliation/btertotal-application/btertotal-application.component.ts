...
@Injectable()
  ...
  @Component({
    selector: 'app-btertotal-application',
    templateUrl: './btertotal-application.component.html',
    styleUrls: ['./btertotal-application.component.css']
  })
export class BTERTotalApplicationComponent implements OnInit {
  ...
  public TotalBTERApplicationList: any = [];
  ...
  async ngOnInit() {
    ...
    await this.GetTotalBterApplicationList();
  }
  ...
  async GetTotalBterApplicationList() {
    debugger;
    try {
      this.loaderService.requestStarted();
      ...
      await this.collegeservice.TotalBTERApplicationDetailsByDepartment(this.request, this.sSOLoginDataModel.SessionID,'ALL')
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          this.TotalBTERApplicationList = data['Data'][0]['data'];
  ...
  btnExportTable_Click(): void {
    this.loaderService.requestStarted();
    if (this.TotalBTERApplicationList.length > 0) {
      try {
