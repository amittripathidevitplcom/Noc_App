...
@Injectable()
  ...
  @Component({
    selector: 'app-btertotal-new-application',
    templateUrl: './btertotal-new-application.component.html',
    styleUrls: ['./btertotal-new-application.component.css']
  })
export class BTERTotalNewApplicationComponent implements OnInit {
  ...
  public TotalBTERNewApplicationList: any = [];
  ...
  async GetTotalBternewApplicationList() {
    debugger;
    try {
      this.loaderService.requestStarted();
      this.request.DepartmentID = this.sSOLoginDataModel.DepartmentID;
      await this.collegeservice.TotalBTERApplicationDetailsByDepartment(this.request, this.sSOLoginDataModel.SessionID, 'New')
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          this.TotalBTERNewApplicationList = data['Data'][0]['data'];
  ...
  btnExportTable_Click(): void {
    this.loaderService.requestStarted();
    if (this.TotalBTERNewApplicationList.length > 0) {
      try {
