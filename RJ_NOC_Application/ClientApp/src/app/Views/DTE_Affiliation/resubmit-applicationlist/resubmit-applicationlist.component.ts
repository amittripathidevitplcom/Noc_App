...
@Injectable()
  ...
  @Component({
    selector: 'app-resubmit-applicationlist',
    templateUrl: './resubmit-applicationlist.component.html',
    styleUrls: ['./resubmit-applicationlist.component.css']
  })
export class ResubmitApplicationlistComponent implements OnInit {
  ...
  async GetTotalBterpendingtoreceivedApplicationList() {
    ...
    try {
      ...
      await this.collegeservice.TotalBTERApplicationDetailsByDepartment(this.request, this.sSOLoginDataModel.SessionID, 'TotalResubmitApplication')
        .then((data: any) => {
          debugger;
          data = JSON.parse(JSON.stringify(data));
