...
@Injectable()
  ...
  @Component({
    selector: 'app-bterreceived-applications',
    templateUrl: './bterreceived-applications.component.html',
    styleUrls: ['./bterreceived-applications.component.css']
  })
export class BTERReceivedApplicationsComponent implements OnInit {
  ...
  async GetTotalBterpendingtoreceivedApplicationList() {
    ...
    try {
      ...
      await this.collegeservice.TotalBTERApplicationDetailsByDepartment(this.request, this.sSOLoginDataModel.SessionID, 'TotalreceivedApplication')
        .then((data: any) => {
          debugger;
          data = JSON.parse(JSON.stringify(data));
          this.TotalBTERreceivedApplicationList = data['Data'][0]['data'];
