...
@Injectable()
  ...
  @Component({
    selector: 'app-btertotal-pendingtosubmit-application',
    templateUrl: './btertotal-pendingtosubmit-application.component.html',
    styleUrls: ['./btertotal-pendingtosubmit-application.component.css']
  })
export class BTERTotalPendingtosubmitApplicationComponent implements OnInit {
  ...
  public TotalBTERPendingtoreceivedApplicationList: any = [];
  ...
  async GetTotalBterpendingtoreceivedApplicationList() {  
    try {
      ...
      await this.collegeservice.TotalBTERApplicationDetailsByDepartment(this.request, this.sSOLoginDataModel.SessionID, 'Pendingtoreceived')
        .then((data: any) => {         
          data = JSON.parse(JSON.stringify(data));
          this.TotalBTERPendingtoreceivedApplicationList = data['Data'][0]['data'];
  ...
  btnExportTable_Click(): void {
    this.loaderService.requestStarted();
    if (this.TotalBTERPendingtoreceivedApplicationList.length > 0) {
      try {
  ...
  async ReceivedApplication() {
    ...
    if (confirm("Are you sure you want to Submitted Application?")) {
      ...
      // this.isLoading = true;
      try {
        await this.commonMasterService.BTERAffiliationFinalSubmit(this.EnterInwordNo, this.ApplicationDateofReceived, this.SelectedDepartmentID, this.SelectedDTEAffiliationID, result, this.SelectedCollageID,'Application Receive')
          .then((data: any) => {
            this.State = data['State'];
            this.SuccessMessage = data['SuccessMessage'];
            this.ErrorMessage = data['ErrorMessage'];
  ...
  async Resetcontroll() {
    this.EnterInwordNo = '';
    this.ApplicationDateofReceived=''
  }
