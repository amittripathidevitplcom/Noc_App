...
@Injectable()
  ...
  @Component({
    selector: 'app-btertotal-submited-application',
    templateUrl: './btertotal-submited-application.component.html',
    styleUrls: ['./btertotal-submited-application.component.css']
  })
export class BTERTotalSubmitedApplicationComponent implements OnInit {
  ...
  public TotalBTERSubmittedApplicationList: any = [];
  ...
  async ngOnInit() {
    ...
    await this.GetTotalBterSubmittedApplicationList();
  }
  ...
  async GetTotalBterSubmittedApplicationList() {
    debugger;
    try {
      this.loaderService.requestStarted();
      this.request.DepartmentID = this.sSOLoginDataModel.DepartmentID;
      await this.collegeservice.TotalBTERApplicationDetailsByDepartment(this.request, this.sSOLoginDataModel.SessionID, 'Submitted')
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          this.TotalBTERSubmittedApplicationList = data['Data'][0]['data'];
  ...
  btnExportTable_Click(): void {
    this.loaderService.requestStarted();
    if (this.TotalBTERSubmittedApplicationList.length > 0) {
      try {
