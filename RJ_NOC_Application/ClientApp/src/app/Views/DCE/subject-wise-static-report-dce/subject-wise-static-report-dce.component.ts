...
@Component({
  selector: 'app-subject-wise-static-report-dce',
  templateUrl: './subject-wise-static-report-dce.component.html',
  styleUrls: ['./subject-wise-static-report-dce.component.css']
})
export class SubjectWiseStaticReportDCEComponent implements OnInit {
  public State: number = -1;
  public SuccessMessage: any = [];
  public ErrorMessage: any = [];
  /*Save Data Model*/
  public SubjectWiseStaticReportLst: any = [];
  ...
  async ngOnInit() {
    ...
    await this.GetSubjectWiseStaticReportList();
    ...
    try {
      ...
      await this.dceDocumentScrutinyService.GetSubjectWiseStaticReport(this.request)
        .then((data: any) => {
          ...
          this.SuccessMessage = data['SuccessMessage'];
          this.ErrorMessage = data['ErrorMessage'];
          this.SubjectWiseStaticReportLst = data['Data'][0]['data'];
        }, error => console.error(error));
    }
    catch (Ex) {
      console.log(Ex);
  ...
  btnExportTable_Click(): void {
    this.loaderService.requestStarted();
    if (this.SubjectWiseStaticReportLst.length > 0) {
      try {
        ...
        /* save to file */
        XLSX.writeFile(wb, "SubjectWiseStaticReportLst.xlsx");
      }
      catch (Ex) {
        console.log(Ex);
      }
