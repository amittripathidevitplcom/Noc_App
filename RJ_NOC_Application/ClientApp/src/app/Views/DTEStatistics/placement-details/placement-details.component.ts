...
import { PlacementDetailsDataModel, PlacementDetailsDataModel_PlacementDetails } from '../../../Models/DTEStatistics/PlacementDetailsDataModel';
...
export class PlacementDetailsComponent implements OnInit {
  ...
  request = new PlacementDetailsDataModel();
  request_PlacementDetailsDetails = new PlacementDetailsDataModel_PlacementDetails();
  ...
  constructor(private PlacementDetailsService: PlacementDetailsService, private loaderService: LoaderService, private router: ActivatedRoute, private commonMasterService: CommonMasterService, private routers: Router, private formBuilder: FormBuilder, private toastr: ToastrService
  ...
  async ngOnInit() {
    ...
    await this.btnAdd_Click(this.request.PlacementDetails[0], 0);
    await this.GetByID();
  }
  async GetProgramme() {
    try {
  ...
  async GetByID() {
    try {
      this.loaderService.requestStarted();
      await this.PlacementDetailsService.GetByID(this.request.CollegeID, this.request.ModifyBy, this.request.EntryType)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          ...
          if (data['Data'].PlacementDetails.length > 0) {
            this.request.PlacementDetails = data['Data'].PlacementDetails;
          }
  ...
  async SaveData() {
    ...
    for (var i = 0; i < this.request.PlacementDetails.length; i++) {
    ...
    this.isLoading = true;
    try {
      await this.PlacementDetailsService.SaveData(this.request)
        .then(async (data: any) => {
          this.State = data['State'];
          this.SuccessMessage = data['SuccessMessage'];
          this.ErrorMessage = data['ErrorMessage'];
  ...
  async btnDelete_Click(i: number) {
    ...
    try {
      if (confirm("Are you sure you want to delete this ?")) {
        this.loaderService.requestStarted();
        this.request.PlacementDetails.splice(i, 1);
      }
    }
    catch (ex) { }
    finally {
  ...
  async btnAdd_Click(row: PlacementDetailsDataModel_PlacementDetails, idx: number) {
    if (row != undefined) {
    ...
    try {
      this.request.PlacementDetails.push({
