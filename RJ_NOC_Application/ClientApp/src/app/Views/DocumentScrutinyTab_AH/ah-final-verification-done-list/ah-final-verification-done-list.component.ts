...
@Component({
  selector: 'app-ah-final-verification-done-list',
  templateUrl: './ah-final-verification-done-list.component.html',
  styleUrls: ['./ah-final-verification-done-list.component.css']
})
export class AhFinalVerificationDoneListComponent {
  ...
  async ngOnInit() {
    ...
    await this.GetFinalVerificationDoneList(this.sSOLoginDataModel.SSOID, this.sSOLoginDataModel.UserID, Number(this.sSOLoginDataModel.RoleID), this.sSOLoginDataModel.DepartmentID, this.QueryStringStatus);
  ...
  async GetFinalVerificationDoneList(SSOID: string, UserID: number, RoleID: number, DepartmentID: number, QueryStringStatus: string) {
    try {
      this.loaderService.requestStarted();
      await this.animalDocumentScrutinyService.GetFinalVerificationDoneList(SSOID, UserID, RoleID, DepartmentID, QueryStringStatus)
        .then((data: any) => {
  ...
  async GetRNCCheckListByTypeDepartment(ApplyNOCID: number) {
    try {
      ...
      this.ActionName = '';
      this.loaderService.requestStarted();
      await this.animalDocumentScrutinyService.GetPreVerificationchecklistDetails('FVCPC', this.sSOLoginDataModel.DepartmentID, ApplyNOCID, this.sSOLoginDataModel.UserID, this.sSOLoginDataModel.RoleID)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          this.State = data['State'];
          this.SuccessMessage = data['SuccessMessage'];
  ...
  async SubmitPhysicalVerification() {
    ...
    try {
      ...
      if (this.ActionID == 0) {
        ...
        if (this.ActionID == 2) {
          this.ActionName = 'Final Verification Rejected';
          ...
          this.ActionName = 'Final Verification Approved';
        }
      }
      if (this.ActionName != '') {
        ...
        this.toastr.warning("Select Final Verification Application Action")
    }
    catch (Ex) {
      console.log(Ex);
    }
