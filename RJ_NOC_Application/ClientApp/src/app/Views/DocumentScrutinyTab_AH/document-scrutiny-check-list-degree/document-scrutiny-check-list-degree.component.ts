...
@Component({
  selector: 'app-document-scrutiny-check-list-degree',
  templateUrl: './document-scrutiny-check-list-degree.component.html',
  styleUrls: ['./document-scrutiny-check-list-degree.component.css']
})
export class DocumentScrutinyCheckListDegreeComponent implements OnInit {
  ...
  async DocumentScrutiny() {
    ...
    try {
      ...
      if (this.sSOLoginDataModel.RoleID == 8 || this.sSOLoginDataModel.RoleID == 43 || this.sSOLoginDataModel.RoleID == 16) {
      ...
      if (this.SelectedDepartmentID == 2) {
        ...
        //else {
        if (this.CheckTabsEntryData['LegalEntity'] <= 0 || this.CheckTabsEntryData['CollegeDetail'] <= 0 || this.CheckTabsEntryData['CollegeManagementSociety'] <= 0 || this.CheckTabsEntryData['LandInformation'] <= 0
          || this.CheckTabsEntryData['Facility'] <= 0 || this.CheckTabsEntryData['OtherInformation'] <= 0
          || this.CheckTabsEntryData['BuildingDocuments'] <= 0 || this.CheckTabsEntryData['RequiredDocument'] <= 0 || this.CheckTabsEntryData['StaffDetails'] <= 0 || this.CheckTabsEntryData['AHInfra'] <= 0) {
          this.isFormvalid = false;
          var tab = this.CheckTabsEntryData['LegalEntity'] <= 0 ? 'Legal Entity' :
            this.CheckTabsEntryData['CollegeDetail'] <= 0 ? 'College Detail' :
              this.CheckTabsEntryData['CollegeManagementSociety'] <= 0 ? 'College Management Society' :
                this.CheckTabsEntryData['LandInformation'] <= 0 ? 'Land Information' :
                  this.CheckTabsEntryData['Facility'] <= 0 ? 'Facility Detail' :
                    this.CheckTabsEntryData['OtherInformation'] <= 0 ? 'Other Infrastructure' :
                      this.CheckTabsEntryData['BuildingDocuments'] <= 0 ? 'Building Details' :
                        this.CheckTabsEntryData['RequiredDocument'] <= 0 ? 'Required Document' :
                          this.CheckTabsEntryData['StaffDetails'] <= 0 ? 'Staff Details' :
                            this.CheckTabsEntryData['AHInfra'] <= 0 ? 'Department Wise Infrastructure' : '';
  ...
  async GetApplicationCommitteeList(ApplyNocApplicationID: number) {
    try {
      this.loaderService.requestStarted();
      await this.committeeMasterService.GetApplicationCommitteeList_AH(ApplyNocApplicationID, this.sSOLoginDataModel.RoleID == 8 ? 'Scrutiny' : this.sSOLoginDataModel.RoleID == 43 ? 'NodalScrutiny' : this.sSOLoginDataModel.RoleID == 16 ? 'Inspection' : '')
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          this.State = data['State'];
          this.SuccessMessage = data['SuccessMessage'];
