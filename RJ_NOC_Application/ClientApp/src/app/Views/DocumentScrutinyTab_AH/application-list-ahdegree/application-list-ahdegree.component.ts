...
@Component({
  selector: 'app-application-list-ahdegree',
  templateUrl: './application-list-ahdegree.component.html',
  styleUrls: ['./application-list-ahdegree.component.css']
})
export class ApplicationListAHDegreeComponent implements OnInit {
  ...
  async GetApplyNOCApplicationListByRole() {
    try {
      ...
      if (this.QueryStatus == 'ScrutinyPending') {
        this.request.ActionName = 'Forward To,Forward';
      }
      ...
      if (this.QueryStatus == 'DCAfterPending') {
        this.request.ActionName = 'Forward To Nodal';
      } if (this.QueryStatus == 'DCAfterCompleted') {
        this.request.ActionName = 'Forward To Director';
      } 
  ...
  async DocumentScrutinyForward(DepartmentID: number, CollegeID: number, ApplyNOCID: number) {
    ...
    this.routers.navigate(['/forwardahdegreecollege' + "/" + encodeURI(this.commonMasterService.Encrypt(DepartmentID.toString())) + "/" + encodeURI(this.commonMasterService.Encrypt(CollegeID.toString())) + "/" + encodeURI(this.commonMasterService.Encrypt(ApplyNOCID.toString()))+"/" + encodeURI(this.commonMasterService.Encrypt(this.request.Status.toString()))]);
  }
  ...
  async GetApplicationCommitteeList(ApplyNocApplicationID: number) {
    ...
    try {
      this.loaderService.requestStarted();
      await this.committeeMasterService.GetApplicationCommitteeList_AH(ApplyNocApplicationID, this.sSOLoginDataModel.RoleID == 42 ? 'Scrutiny' :'NodalScrutiny')
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          this.State = data['State'];
          this.SuccessMessage = data['SuccessMessage'];
    ...
    if (!isValidate && !isValidateMobile) {
      ...
      if (this.AadhaarNo.length > 0) {
        this.request_MemberList.ApplicationCommitteeList.push({
          ...
          CommitteeType: this.sSOLoginDataModel.RoleID == 42 ? 'Scrutiny' : 'NodalScrutiny',
        });
