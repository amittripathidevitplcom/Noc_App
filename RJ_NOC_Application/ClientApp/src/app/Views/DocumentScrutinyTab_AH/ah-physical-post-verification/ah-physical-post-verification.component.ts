...
@Component({
  ...
  templateUrl: './ah-physical-post-verification.component.html',
  styleUrls: ['./ah-physical-post-verification.component.css']
})
export class AhPhysicalPostVerificationComponent {
  ...
  async GetPostVerificationAppliationList(SSOID: string, UserID: number, RoleID: number, DepartmentID: number, QueryStringStatus: string) {
    try {
      ...
      await this.animalDocumentScrutinyService.GetPostVerificationAppliationList(SSOID, UserID, RoleID, DepartmentID, QueryStringStatus)
        .then((data: any) => {
