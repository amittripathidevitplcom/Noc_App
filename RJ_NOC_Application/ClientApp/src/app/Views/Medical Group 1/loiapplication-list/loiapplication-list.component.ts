...
@Injectable()
  @Component({
    selector: 'app-loiapplication-list',
    templateUrl: './loiapplication-list.component.html',
    styleUrls: ['./loiapplication-list.component.css']
  })

export class LOIApplicationListComponent implements OnInit {
  //Add FormBuilder
  ...
  public loiApplicatoinListData: any = [];
  searchText: string = '';
...
private commonMasterService: CommonMasterService, private router: ActivatedRoute, private routers: Router) {
  ...
  async GetApplicationList() {
    try {
      ...
      await this.draftApplicationListService.LOIApplicationList(this.sSOLoginDataModel.SSOID)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          this.State = data['State'];
          this.SuccessMessage = data['SuccessMessage'];
          this.ErrorMessage = data['ErrorMessage'];
          this.loiApplicatoinListData = data['Data'][0]['data'];
