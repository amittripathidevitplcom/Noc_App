...
@Component({
  selector: 'app-legal-entity-revert',
  templateUrl: './legal-entity-revert.component.html',
  styleUrls: ['./legal-entity-revert.component.css']
})
export class LegalEntityRevertComponent implements OnInit {
  ...
  async GetCollageDetails() {
    try {
      this.loaderService.requestStarted();
      await this.collegeService.GetData(this.SelectedCollegeID)
        .then(async (data: any) => {
          data = JSON.parse(JSON.stringify(data));
          if (data != null) {
            debugger;
