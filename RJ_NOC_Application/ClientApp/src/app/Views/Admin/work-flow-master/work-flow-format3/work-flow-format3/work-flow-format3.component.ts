...
export class WorkFlowFormat3Component implements OnInit {
  ...
  constructor(private loaderService: LoaderService, private workFlowMasterService: WorkFlowMasterService, private routers: Router) { }
  ...
  //async OpenWorkFlowMasterDetailsModel(DepartmentID: number) {
 
  // try {
  //    this.loaderService.requestStarted();
  //   await this.workFlowMasterService.GetWorkFlowformat3(DepartmentID)
  //.then((data: any) => {
  //  data = JSON.parse(JSON.stringify(data));
  //  this.WorkFlowFormat3Data = data['Data'];
  //}, error => console.error(error));
  ...
  //async OpenWorkFlowMasterDetailsModel(event: any, SeletedValueState: any) {
  //  this.request.DepartmentID = SeletedValueDepartment;
  //  try {
  //    //this.loaderService.requestStarted();
  //    await this.workFlowMasterService.GetWorkFlowformat3(this.request.DepartmentID)
  //      .then((data: any) => {
  //        data = JSON.parse(JSON.stringify(data));
  //        this.State = data['State'];
  //        this.SuccessMessage = data['SuccessMessage'];
