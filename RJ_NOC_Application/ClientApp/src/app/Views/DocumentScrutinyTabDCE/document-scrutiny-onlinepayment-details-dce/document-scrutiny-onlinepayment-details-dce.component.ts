...
@Component({
  selector: 'app-document-scrutiny-onlinepayment-details-dce',
  templateUrl: './document-scrutiny-onlinepayment-details-dce.component.html',
  styleUrls: ['./document-scrutiny-onlinepayment-details-dce.component.css']
})
export class DocumentScrutinyOnlinepaymentDetailsDceComponent implements OnInit {
  ...
  async SubmitOffPaymentDetail_Onclick() {
    ...
    this.dsrequest.TabName = 'OnlinePayment';
    this.isRemarkValid = false;
    this.isFormvalid = true;
    this.dsrequest.DocumentScrutinyDetail = [];
    for (var i = 0; i < this.paymentResponseDataModel.length; i++) {
      if (this.paymentResponseDataModel[i].Action == '' || this.paymentResponseDataModel[i].Action == undefined) {
        this.toastr.warning('Please take Action on all records');
        return;
      }
      if (this.paymentResponseDataModel[i].Action == 'No') {
        if (this.paymentResponseDataModel[i].Remark == '' || this.paymentResponseDataModel[i].Remark == undefined) {
    ...
    if (this.paymentResponseDataModel.length > 0) {
      for (var i = 0; i < this.paymentResponseDataModel.length; i++) {
        console.log(this.paymentResponseDataModel[i]);
        this.dsrequest.DocumentScrutinyDetail.push({
          ...
          Action: this.paymentResponseDataModel[i].Action,
          Remark: this.paymentResponseDataModel[i].Remark,
          TabRowID: this.paymentResponseDataModel[i].PaymentID,
          SubTabName: ''
        });
      }
    }
