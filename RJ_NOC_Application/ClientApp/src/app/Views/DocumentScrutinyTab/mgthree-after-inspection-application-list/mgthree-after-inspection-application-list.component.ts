...
import { ApplyNOCDocumentFilter_DataModel, ApplyNOCDocument_DataModel, CommiteeInspection_RNCCheckList_DataModel, CommonDataModel_ApplicationListFilter } from '../../../Models/ApplyNOCApplicationDataModel';
...
export class MGThreeAfterInspectionApplicationListComponent implements OnInit {
  ...
  async OpenCheckList(content: any, ApplyNOCID: number, DepartmentID: number, CollegeID: number, ApplicationNo: string) {
    await this.GetApplyNOCDocumentList(ApplyNOCID, 'InspectionDocument');
    if (this.nocdocumentlst.length == 0) {
      this.nocdocumentlst.push({
        ApplyNOCID: ApplyNOCID,
        DocumentID: 0,
        DocumentType: 'InspectionDocument',
        DocumentName: '',
        DocumentName_DisName: '',
        DocumentNamePath: '',
  ...
  public nocdocumentlst: ApplyNOCDocument_DataModel[] = [];

  AddMoreDocuments() {
    for (var i = 0; i < this.nocdocumentlst.length; i++) {
      if (this.nocdocumentlst[i].DocumentName == '') {
        this.toastr.warning('Please upload a document');
        return;
      }
    }
  ...
  async DeleteDocuments(i: number) {
    try {
      if (confirm("Are you sure you want to delete this ?")) {
        this.loaderService.requestStarted();
        this.nocdocumentlst.splice(i, 1);
      }
    }
    catch (ex) { }
    finally {
  ...
  async UploadDocument(event: any, Type: string, Controlname: string, idx: number) {
    try {
      this.loaderService.requestStarted();
      this.file = event.target.files[0];
      if (this.file) {
        if (this.file.type == 'application/pdf' || this.file.type == 'image/jpeg' || this.file.type == 'image/jpg') {
          //size validation
          if (this.file.size > 2000000) {
            this.ResetFile(Type, '', '', '', Controlname, 'Select less then 2MB File', idx);
            return
          }
          //if (this.file.size < 100000) {
          //  this.ResetFile(Type, '', '', '', Controlname, 'Select more then 100kb File', idx);
          //  return
          //}
        }
        else {// type validation
          this.ResetFile(Type, '', '', '', Controlname, 'Select Only pdf/jpeg/jpg file', idx);
          return
        }
        // upload to server folder
        await this.fileUploadService.UploadDocument(this.file).then((data: any) => {
          ...
          this.ErrorMessage = data['ErrorMessage'];
          if (this.State == 0) {
            this.ResetFile(Type, data['Data'][0]["FileName"], data['Data'][0]["FilePath"], data['Data'][0]["Dis_FileName"], Controlname, '', idx);
          }
          if (this.State == 1) {
            this.toastr.error(this.ErrorMessage)
          }
  ...
  async DeleteDocument(file: string, Type: string, Controlname: string, idx: number) {
    try {
      ...
      if (confirm("Are you sure you want to delete this ?")) {
        ...
        await this.fileUploadService.DeleteDocument(file).then((data: any) => {
          ...
          this.ErrorMessage = data['ErrorMessage'];
          if (this.State == 0) {
            this.ResetFile(Type, '', '', '', Controlname, '', idx);
          }
          if (this.State == 1) {
            this.toastr.error(this.ErrorMessage)
          }
  ...
  async ResetFile(Type: string, FileName: string, FilePath: string, DisName: string, ControlName: string, Message: string, idx: number) {
    ...
    this.nocdocumentlst[idx].DocumentName = FileName;
    this.nocdocumentlst[idx].DocumentName_DisName = DisName;
    this.nocdocumentlst[idx].DocumentNamePath = FilePath;
  ...
  async SaveDocuments() {
    ...
    const updatelst = this.nocdocumentlst.filter(items => items.DocumentName != '');
    if (updatelst.length <= 0) {
      this.toastr.warning('Please upload atleast one document');
      return
    }
    this.nocdocumentlst = this.nocdocumentlst.filter(items => items.DocumentName != '');
    for (var i = 0; i < this.nocdocumentlst.length; i++) {
      this.nocdocumentlst[i].CreatedBy = this.sSOLoginDataModel.UserID;
      this.nocdocumentlst[i].RoleID = this.sSOLoginDataModel.RoleID;
    }
    this.isSubmitted = true;
    try {
      this.loaderService.requestStarted();
      await this.commonMasterService.SaveApplyNOCDocumentList(this.nocdocumentlst)
        .then(async (data: any) => {
          data = JSON.parse(JSON.stringify(data));
          this.State = data['State'];
          this.SuccessMessage = data['SuccessMessage'];
  ...
  public applynocdocumentfilter = new ApplyNOCDocumentFilter_DataModel();
  async GetApplyNOCDocumentList(ApplyNOCID: number, DocumentType: string) {
    try {
      this.applynocdocumentfilter.ApplyNOCID = ApplyNOCID;
      this.applynocdocumentfilter.DocumentType = DocumentType;
      this.loaderService.requestStarted();
      await this.commonMasterService.GetApplyNOCDocumentList(this.applynocdocumentfilter)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          this.State = data['State'];
          this.SuccessMessage = data['SuccessMessage'];
          this.ErrorMessage = data['ErrorMessage'];
          this.nocdocumentlst = data['Data'][0]['data'];
          for (var i = 0; i < this.nocdocumentlst.length; i++) {
            if (i != 0) {
              this.nocdocumentlst[i].Showbutton = true;
            }
          }
        }, error => console.error(error));
    }
  ...
  public AllDocumentsList: any = [];
  async GetApplyNOCAllDocumentList(content: any, ApplyNOCID: number, DocumentType: string) {
    try {
      ...
      await this.commonMasterService.GetApplyNOCDocumentList(this.applynocdocumentfilter)
        .then((data: any) => {
          ...
          this.SuccessMessage = data['SuccessMessage'];
          this.ErrorMessage = data['ErrorMessage'];
          this.AllDocumentsList = data['Data'][0]['data'];
        }, error => console.error(error));
      this.modalService.open(content, { size: 'xl', ariaLabelledBy: 'modal-basic-title', backdrop: 'static' }).result.then((result) => {
        this.closeResult = `Closed with: ${result}`;
      }, (reason) => {
