...
@Component({
  selector: 'app-dtcoursemaster',
  templateUrl: './dtcoursemaster.component.html',
  styleUrls: ['./dtcoursemaster.component.css']
})
export class DtcoursemasterComponent {
  ...
  async GetCourseListByLevelID(CourseLevelID: number) {
    ...
    try {
      this.loaderService.requestStarted();
      await this.commonMasterService.GetCourseByStreamID(this.request.StreamID, 4, 0, 0)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          this.State = data['State'];
          this.SuccessMessage = data['SuccessMessage'];
  ...
  async CourseLevel() {
    try {
      this.loaderService.requestStarted();
      await this.commonMasterService.GetCommonMasterList_DepartmentAndTypeWises(4, 4511, "CourseLevel")
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          this.State = data['State'];
          this.SuccessMessage = data['SuccessMessage'];
  ...
  async GetAllCourseList() {
    try {
      this.loaderService.requestStarted();
      await this.courseMasterService.GetAllCourseList(this.UserID, 4)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
  ...
  btnExportTable_Click(): void {
    ...
    if (this.CourseDataList.length > 0) {
      try {
        ...
        /* save to file */
        XLSX.writeFile(wb, "DTCourseMaster.xlsx");
      }
      catch (Ex) {
        console.log(Ex);
      }
