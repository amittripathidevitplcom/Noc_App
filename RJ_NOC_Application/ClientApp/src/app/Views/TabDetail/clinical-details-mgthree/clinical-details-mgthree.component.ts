...
@Component({
  selector: 'app-clinical-details-mgthree',
  templateUrl: './clinical-details-mgthree.component.html',
  styleUrls: ['./clinical-details-mgthree.component.css']
})

export class ClinicalDetailsMGThreeComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (this.SelectedDepartmentID == 2) {
      ...
      this.textHeading = 'Clinical Details';
    }   else {
  ...
  public CollegeCourseDataList: any = [];
  async GetCourseList() {
    this.CollegeCourseDataList = [];
    try {
      this.loaderService.requestStarted();
      await this.commonMasterService.GetCourseList_CollegeWise(this.SelectedCollageID)
        .then(async (data: any) => {
          data = JSON.parse(JSON.stringify(data));
          this.CollegeCourseDataList = data['Data'];
  ...
  async ddlCourse_newchange(CourseID: number, OtherInformationID: number) {
    try {
      this.loaderService.requestStarted();
      this.courseDataList = [];
      this.request.CourseID = 0;
      await this.commonMasterService.OtherInformationList_CourseID(CourseID, this.SelectedCollageID, OtherInformationID)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
    ...
    try {
      ...
      await this.otherInformationService.GetOtherInformationByID(CollegeWiseOtherInfoID, this.UserID, this.SelectedCollageID)
        .then(async (data: any) => {
          ...
          this.request.NewCourseID = data['Data'][0]["NewCourseID"];
          await this.ddlCourse_newchange(data['Data'][0]["NewCourseID"], data['Data'][0]["CourseID"]);
