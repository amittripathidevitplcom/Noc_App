...
import { EnumCheckListType_Agri, EnumCommitteType } from '../../../Common/enum-noc';

@Component({
  selector: 'app-agri-insepction-post-verification',
  templateUrl: './agri-insepction-post-verification.component.html',
  styleUrls: ['./agri-insepction-post-verification.component.css']
})
export class AgriInsepctionPostVerificationComponent {
  ...
  async ngOnInit() {
    ...
    await this.GetPostVerificationAppliationList(this.sSOLoginDataModel.SSOID, this.sSOLoginDataModel.UserID, Number(this.sSOLoginDataModel.RoleID), this.sSOLoginDataModel.DepartmentID, this.QueryStringStatus);
  ...
  async GetPostVerificationAppliationList(SSOID: string, UserID: number, RoleID: number, DepartmentID: number, QueryStringStatus: string) {
    try {
      this.loaderService.requestStarted();
      await this.agricultureDocumentScrutinyService.GetPostVerificationAppliationList(SSOID, UserID, RoleID, DepartmentID, QueryStringStatus)
        .then((data: any) => {
  ...
  async GetRNCCheckListByTypeDepartment(ApplyNOCID: number) {
    try {
      this.loaderService.requestStarted();
      await this.commonMasterService.GetRNCCheckListByTypeDepartment(EnumCheckListType_Agri.PIPV.toString(), this.sSOLoginDataModel.DepartmentID, ApplyNOCID, this.sSOLoginDataModel.UserID, this.sSOLoginDataModel.RoleID)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          this.State = data['State'];
          this.SuccessMessage = data['SuccessMessage'];
  ...
  async SubmitPhysicalVerification() {
    ...
    try {
      ...
      await this.agricultureDocumentScrutinyService.FinalSubmitInspectionCommittee(this.ApplicationCommitteeList[0].ApplyNocApplicationID, this.sSOLoginDataModel.DepartmentID, this.sSOLoginDataModel.UserID, "Post Verification Forward")
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          this.State = data['State'];
          this.SuccessMessage = data['SuccessMessage'];
