...
@Component({
  selector: 'app-agri-ds-committee-primary-verification',
  templateUrl: './agri-ds-committee-primary-verification.component.html',
  styleUrls: ['./agri-ds-committee-primary-verification.component.css']
})
export class AgriDsCommitteePrimaryVerificationComponent {
  ...
  async GetPhysicalVerificationAppliationList(SSOID: string, UserID: number, RoleID: number, DepartmentID: number, QueryStringStatus: string) {
    try {
      ...
      await this.agricultureDocumentScrutinyService.GetPhysicalVerificationAppliationList(SSOID, UserID, RoleID, DepartmentID, QueryStringStatus)
        .then((data: any) => {
  ...
  async GetRNCCheckListByTypeDepartment(ApplyNOCID: number) {
    try {
      this.loaderService.requestStarted();
      await this.commonMasterService.GetRNCCheckListByTypeDepartment(EnumCheckListType_Agri.DSPV, this.sSOLoginDataModel.DepartmentID, ApplyNOCID, this.sSOLoginDataModel.UserID, this.sSOLoginDataModel.RoleID)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          this.State = data['State'];
          this.SuccessMessage = data['SuccessMessage'];
  ...
  async SubmitPhysicalVerification() {
    ...
    try {
      ...
      this.loaderService.requestStarted();
      await this.agricultureDocumentScrutinyService.FinalSubmitInspectionCommittee(this.ApplicationCommitteeList[0].ApplyNocApplicationID, this.sSOLoginDataModel.DepartmentID, this.sSOLoginDataModel.UserID, "Pre Verification Forward")
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          this.State = data['State'];
          this.SuccessMessage = data['SuccessMessage'];
