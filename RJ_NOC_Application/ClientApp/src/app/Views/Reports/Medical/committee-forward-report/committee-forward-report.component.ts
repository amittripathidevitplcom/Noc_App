...
@Component({
  selector: 'app-committee-forward-report',
  templateUrl: './committee-forward-report.component.html',
  styleUrls: ['./committee-forward-report.component.css']
})
export class CommitteeForwardReportComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.SelectedCommittee = this.commonMasterService.Decrypt(this.router.snapshot.paramMap.get('Committee')?.toString());
  ...
  async GetApplyNOCApplicationListByRole(RoleId: number, UserID: number) {
    try {
      ...
      var Action = this.SelectedCommittee == 'Scrutiny' ? 'Forward To Secretary By Scrutiny Committee' : this.SelectedCommittee == 'Registrat' ? '' : '';
      await this.applyNOCApplicationService.GetPendingMedicalApplications(RoleId, UserID, Action)
        .then((data: any) => {
