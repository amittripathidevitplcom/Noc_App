...
@Component({
  selector: 'app-joint-secretary-completed-report',
  templateUrl: './joint-secretary-completed-report.component.html',
  styleUrls: ['./joint-secretary-completed-report.component.css']
})
export class JointSecretaryCompletedReportComponent implements OnInit {
  ...
  public ApplicationCompleteList: any = [];
  ...
  async GetApplicationCompletedReportUserWise(UserID: number, RoleID: number, DepartmentID: number) {
    try {
      ...
      await this.applyNOCApplicationService.GetApplyNOCCompletedReport(UserID, 'Forward To Secretary', RoleID, DepartmentID)
        .then((data: any) => {
          ...
          if (data['Data'][0]['data'].length > 0) {
            this.ApplicationCompleteList = data['Data'][0]['data'];
          }
