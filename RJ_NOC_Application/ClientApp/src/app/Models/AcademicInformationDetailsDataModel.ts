import { DocumentScrutiny_ActionModule} from '../Models/LandDetailDataModel'
export class AcademicInformationDetailsDataModel extends DocumentScrutiny_ActionModule {
  public AcademicInformationID: number = 0;
  public CollegeID: number = 0;
  public YearID: number = 0;
  public YearValue: string = '';
  public CourseID: number = 0;
  public CourseName: string = '';
  public AdmittedStudent: number = null;
  public AppearedStudent: number = null;
  public ResultID: number = 0;
  public ResultName: string = '';
  public PassedStudent: number = null;
  public FailedStudent: number = null;
  public OtherStudent: number = null;
  public Percentage: number = 0;
  public UserID: number = 0;
  public FinancialYearName: string = '';
}
